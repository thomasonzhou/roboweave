...
class KeyboardTeleopLowLevel:
    ...
    def create_low_cmd(self):
        ...
        for i in range(12):
            ...
            cmd.motor_cmd[i].kp = 20.0  # Position gain
            cmd.motor_cmd[i].kd = 0.5   # Damping gain
            
        # Apply movement adjustments when standing
        if self.is_standing:
            # Simple leg length adjustments for movement
            # This is a very basic implementation - real walking would need proper gait generation
            leg_adjustment = 0.1 * self.linear_vel  # Basic forward/backward adjustment
            height_adjustment = self.body_height
            
            # Adjust leg positions based on velocities (very simplified)
            for i in range(0, 12, 3):  # Each leg has 3 joints
                # Hip adjustment for turning
                cmd.motor_cmd[i].q += 0.1 * self.angular_vel * (1 if i < 6 else -1)
                # Thigh adjustment for forward motion
                cmd.motor_cmd[i+1].q += leg_adjustment
                # Calf adjustment for height and forward motion
                cmd.motor_cmd[i+2].q -= 2 * leg_adjustment + height_adjustment

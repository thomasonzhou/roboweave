...
export function RoboWeaveFlowBuilder() {
    const [isMobileView] = useApplicationState(s => [s.view.mobile]);

    const [nodes, _, onNodesChange] = useNodesState<Node>(defaultRoboweaveNodes);
    ...
    return (
        ...
        >
            <Background 
                color={isMobileView ? "rgba(251, 191, 36, 0.15)" : "rgba(251, 191, 36, 0.1)"} 
                gap={32} 
                size={1}
            />
            <CustomControls />
